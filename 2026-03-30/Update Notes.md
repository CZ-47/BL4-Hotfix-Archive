URL: https://support.borderlands.com/hc/en-us/articles/50399116113811-Borderlands-4-Minor-Update-Notes-March-30-2026

Minor Update Notes - March 30, 2026

Today, we released an update to address reported issues that emerged after the release of Story Pack 1: Mad Ellie and the Vault of the Damned. After seeing your reports the team moved immediately into action to investigate, fix, and test over the weekend to correct these issues as soon as possible. 
Update Notes:

    Missing SDU Tokens - Fixed an issue where SDU progress would reset and players would lose some of their tokens when Shared Character Progression automatically applies to a newly created Vault Hunter.
    Please note: After applying the latest Update, all SDUs earned on all characters will be restored, but they also had to be reset. You will need to reassign your SDU Tokens when you log into the game.
    Epic Game Store - Fixed an issue where players who owned the Super Deluxe Version on the Epic Game Store were unable to access Story Pack 1: Mad Ellie and the Vault of the Damned and the new Vault Hunter, C4SH.

Thank you all so much for your patience. We really value and appreciate your replies, comments, threads, and tickets helping us along the way. 

Here’s a SHiFT code for 5 Golden Keys* you can use at Golden Chests in Borderlands 4:
3ZR3T-6CSKW-WFTJC-JB33B-Z55ZT

SHiFT codes can be entered on the Gearbox SHiFT site or via the in-game SHiFT menu in Borderlands 4! This code may be redeemed once (1) per SHiFT account. You can check out the SHiFT User Guide for more info.

*Requires a copy of Borderlands 4, SHiFT Account, and Internet connection. Must redeem by April 30, 2026. One redemption per SHiFT Account. Terms apply.
