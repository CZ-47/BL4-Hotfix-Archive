Update Notes: https://support.borderlands.com/hc/en-us/articles/49260255211027-Borderlands-4-Minor-Update-Notes-February-19-2026
Minor Update Notes - February 19, 2026


This Minor Update includes an update to the loot pool on Moxxi's Tip Jar and a new set of Weekly Activities.

Please restart your game to ensure that the update goes through and to prevent connection errors in matchmaking.

If you're encountering any issues, please contact 2K Support for direct assistance. You can also read the PC Troubleshooting Guide, a list of known issues, or browse a list of Support threads here.

Change Notes:

    The Phosphenes drops for Cold Shoulder and Sweet Embrace have been added to the loot pool on Moxxi's Tip Jar.
    These previously unattainable drops are included in the Phosphene challenge, which should now be possible to complete.

Weekly Activities

Weekly Big Encore Boss:

    Skull Orchid
    
Weekly Wildcard Mission:

    Mission: Old Lads (Shielded, Armored, Fortified)
    Legendary Gear Reward: Divided Focus Tediore Assault Rifle 
